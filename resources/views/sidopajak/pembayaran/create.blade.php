    ...
    <x-slot name="header">
        <div class="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
            <div>
                ...
                <p class="text-sm text-gray-500">Catat transaksi pembayaran SPPT yang lengkap dan aman.</p>
            </div>
                ...
                <form method="POST" action="{{ route('pembayaran.store') }}" class="space-y-4">
                    ...
                    <div class="grid gap-4 sm:grid-cols-2">
                        <div>
                            ...
                            <input type="text" name="id_bayar" value="{{ old('id_bayar') }}"
                        ...
                        <div>
                            ...
                            <select name="id_sppt"
                                ...
                                @foreach ($sppt as $item)
                                    <option value="{{ $item->id_sppt }}" @selected(old('id_sppt') == $item->id_sppt)>
                    ...
                    <div>
                        ...
                        <select name="id_petugas"
                            ...
                            @foreach ($users as $user)
                                <option value="{{ $user->id }}" @selected(old('id_petugas') == $user->id)>{{ $user->username }}
                                    ({{ $user->role }})</option>
                            @endforeach
                        </select>
                    </div>
