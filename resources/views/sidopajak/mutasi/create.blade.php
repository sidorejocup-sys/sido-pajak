<x-app-layout>
    <x-slot name="header">
        <div class="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
            <div>
                <h2 class="font-semibold text-xl text-gray-800 leading-tight">Tambah Riwayat Mutasi</h2>
                <p class="text-sm text-gray-500">Catat perpindahan kepemilikan properti dengan lengkap.</p>
            </div>
                ...
                <form method="POST" action="{{ route('mutasi.store') }}" class="space-y-4">
                    ...
                    <div class="grid gap-4 sm:grid-cols-2">
                        <div>
                            ...
                            <input type="text" name="id_mutasi" value="{{ old('id_mutasi') }}"
                        ...
                        <div>
                            ...
                            <select name="nop_asal"
                                ...
                                @foreach ($objek as $item)
                                    <option value="{{ $item->nop }}" @selected(old('nop_asal') == $item->nop)>{{ $item->nop }}
                    ...
                    <div class="grid gap-4 sm:grid-cols-2">
                        <div>
                            ...
                            <select name="nik_lama"
                                ...
                                @foreach ($subjek as $item)
                                    <option value="{{ $item->nik }}" @selected(old('nik_lama') == $item->nik)>{{ $item->nik }}
                                        - {{ $item->nama }}</option>
                                @endforeach
                            </select>
                        </div>
                        <div>
                            ...
                            <select name="nik_baru"
                                ...
                                <option value="">Pilih subjek pajak</option>
                                @foreach ($subjek as $item)
                                    <option value="{{ $item->nik }}" @selected(old('nik_baru') == $item->nik)>
                    ...
                    <div class="grid gap-4 sm:grid-cols-3">
                        <div>
                            ...
                            <select name="jenis_mutasi"
                                class="mt-1 block w-full rounded-lg border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500"
                                required>
                                <option value="jual" @selected(old('jenis_mutasi') == 'jual')>Jual</option>
                                <option value="hibah" @selected(old('jenis_mutasi') == 'hibah')>Hibah</option>
                                <option value="waris" @selected(old('jenis_mutasi') == 'waris')>Waris</option>
                                <option value="lainnya" @selected(old('jenis_mutasi') == 'lainnya')>Lainnya</option>
                            </select>
                        </div>
                        <div>
                            <label class="block text-sm font-medium text-gray-700">Tanggal Mutasi</label>
                            <input type="date" name="tgl_mutasi" value="{{ old('tgl_mutasi') }}"
                                class="mt-1 block w-full rounded-lg border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500"
                                required />
                        </div>
                        <div>
                            <label class="block text-sm font-medium text-gray-700">No Arsip</label>
                            <input type="text" name="no_arsip" value="{{ old('no_arsip') }}"
                                class="mt-1 block w-full rounded-lg border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500" />
                        </div>
                    </div>
