<x-app-layout>
    <x-slot name="header">
        <div class="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
            <div>
                <h2 class="font-semibold text-xl text-gray-800 leading-tight">Ubah SPPT</h2>
                <p class="text-sm text-gray-500">Perbarui tagihan SPPT sesuai data terbaru.</p>
            </div>
    ...
    <div class="py-6">
        <div class="max-w-3xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white shadow-sm sm:rounded-lg p-6">
                ...
                <form method="POST" action="{{ route('sppt.update', $sppt->id_sppt) }}" class="space-y-4">
                    @csrf
                    @method('PUT')
                    <div class="grid gap-4 sm:grid-cols-2">
                        <div>
                            <label class="block text-sm font-medium text-gray-700">ID SPPT</label>
                            <input type="text" name="id_sppt" value="{{ old('id_sppt', $sppt->id_sppt) }}"
                        ...
                        <div>
                            ...
                            <select name="nop"
                                ...
                                @foreach ($objek as $item)
                                    <option value="{{ $item->nop }}" @selected(old('nop', $sppt->nop) == $item->nop)>{{ $item->nop }}
                    ...
                    <div class="grid gap-4 sm:grid-cols-3">
                        ...
                        <div>
                            ...
                            <input type="number" step="0.01" min="0" name="njop_bumi"
                                value="{{ old('njop_bumi', $sppt->njop_bumi) }}"
                                class="mt-1 block w-full rounded-lg border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500"
                                required />
                        </div>
                        <div>
                            ...
                            <input type="number" step="0.01" min="0" name="njop_bangunan"
                                value="{{ old('njop_bangunan', $sppt->njop_bangunan) }}"
                                class="mt-1 block w-full rounded-lg border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500"
                                required />
                        </div>
                    </div>
                    ...
                    <div class="grid gap-4 sm:grid-cols-2">
                        <div>
                            ...
                            <input type="number" step="0.01" min="0" name="pajak_terhutang"
                                value="{{ old('pajak_terhutang', $sppt->pajak_terhutang) }}"
                                class="mt-1 block w-full rounded-lg border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500"
                                required />
                        </div>
                        <div>
                            ...
                            <select name="status_bayar"
                                class="mt-1 block w-full rounded-lg border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500"
                                required>
                                <option value="piutang" @selected(old('status_bayar', $sppt->status_bayar) == 'piutang')>Piutang</option>
                                <option value="lunas" @selected(old('status_bayar', $sppt->status_bayar) == 'lunas')>Lunas</option>
                            </select>
                        </div>
                    </div>
