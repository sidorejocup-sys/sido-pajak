<x-app-layout>
    <x-slot name="header">
        <div class="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
            <div>
                <h2 class="font-semibold text-xl text-gray-800 leading-tight">Ubah Objek Pajak</h2>
                <p class="text-sm text-gray-500">Perbarui NOP atau informasi pemilik objek pajak.</p>
            </div>
    ...
    <div class="py-6">
        <div class="max-w-3xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white shadow-sm sm:rounded-lg p-6">
                ...
                <form method="POST" action="{{ route('objek-pajak.update', $objek->id) }}" class="space-y-4">
                    @csrf
                    @method('PUT')
                    <div class="grid gap-4 sm:grid-cols-2">
                        <div>
                            <label class="block text-sm font-medium text-gray-700">NOP</label>
                            <input type="text" name="nop" value="{{ old('nop', $objek->nop) }}"
                        ...
                        <div>
                            ...
                            <select name="nik_pemilik"
                                ...
                                @foreach ($subjek as $owner)
                                    <option value="{{ $owner->nik }}" @selected(old('nik_pemilik', $objek->nik_pemilik) == $owner->nik)>{{ $owner->nik }}
                                        - {{ $owner->nama }}</option>
                                @endforeach
                            </select>
                        </div>
                    ...
                    <div>
                        ...
                        <textarea name="letak_objek" rows="3"
                            class="mt-1 block w-full rounded-lg border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500"
                            required>{{ old('letak_objek', $objek->letak_objek) }}</textarea>
                    </div>
                    ...
                    <div class="grid gap-4 sm:grid-cols-3">
                        <div>
                            ...
                            <input type="number" step="0.01" min="0" name="luas_bumi"
                                value="{{ old('luas_bumi', $objek->luas_bumi) }}"
                                class="mt-1 block w-full rounded-lg border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500"
                                required />
                        </div>
                        <div>
                            ...
                            <input type="number" step="0.01" min="0" name="luas_bangunan"
                                value="{{ old('luas_bangunan', $objek->luas_bangunan) }}"
                                class="mt-1 block w-full rounded-lg border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500"
                                required />
                        </div>
                        <div>
                            ...
                            <select name="status_aktif"
                                class="mt-1 block w-full rounded-lg border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500"
                                required>
                                <option value="aktif" @selected(old('status_aktif', $objek->status_aktif) == 'aktif')>Aktif</option>
                                <option value="nonaktif" @selected(old('status_aktif', $objek->status_aktif) == 'nonaktif')>Nonaktif</option>
                            </select>
                        </div>
                    </div>
